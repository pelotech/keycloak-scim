...
| `sync-on-error` | enum | `auto` | How a per-record failure in the sync loop is handled. `auto` (default): a permanent failure (bad mapping, malformed data, 4xx) or a 429 throttling response skips that record and the run continues; a transient failure (endpoint unreachable, 5xx) stops the run, since every remaining record would fail the same way. `continue`: always skip the failed record and keep going. `stop`: abort on the first failure of any kind. Options: `auto`, `continue`, `stop`. |
